...
public class FlightDao {
	...
	public void addFlight(FlightDetail flightDetail) {
		...
		try {
			...
			String sql="insert into flight_detail(country_name,flight_name,source,destination,departure_time,arrival_time,status,journey_date)values(?,?,?,?,?,?,?,?)";
	...
	public void removePackage(FlightDetail flightDetail) {
		...
		try {
			...
			Date journeyDate=Date.valueOf(flightDetail.getJourneyDate());
			connection=ConnectionUtil.getConnection();
			String sql="delete from flight_detail where country_name=? AND flight_name=? AND source=? AND destination=? AND departure_time=? AND arrival_time=? AND status=? AND journey_date=?";
			pst=connection.prepareStatement(sql);
			pst.setString(1,flightDetail.getCountryName());
			pst.setString(2,flightDetail.getFlightName());
			pst.setString(3,flightDetail.getSource());
